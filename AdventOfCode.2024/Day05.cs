...
public class Day05
{
    ...
    private static string Solution1(string input)
    {
        ...
        var emptyLineIdx = lines.FindIndex(string.IsNullOrWhiteSpace);
        var rulesArray = lines.Take(emptyLineIdx).Select(l => l.Split("|")).ToArray();
        ...
        var updates = lines.Skip(emptyLineIdx + 1).Select(l => l.Split(",")).Select(p => p.Select(int.Parse).ToList()).ToList();
        ...
        foreach (var pages in updates)
        {
            ...
            {
                var predecessors = pages.Take(i);
                if (!rules.TryGetValue(pages[i], out var successors)) continue;
                if (!successors.Intersect(predecessors).Any()) continue;
                isValid = false;
                break;
            }
  ...
  var lines = input.Split("\n").Select(l => l.Trim()).ToList();
        ...
        foreach (var pages in updates)
        {
            ...
            var toFix = pages.ToHashSet();
            while (newUpdate.Count < pages.Count)
            {
                int leaf = -1;
                foreach (var node in toFix)
                {
                    if (rules.TryGetValue(node, out var successors))
                    {
                        if (successors.Intersect(toFix).Any())
                        {
            ...
            total += newUpdate[newUpdate.Count / 2];
        }
