...
public class Day01
{
    ...
    private static string Solution1(string input)
    {
        ...
        var leftList = lines.Select(l => l.left).Order().ToList();
        var rightList = lines.Select(l => l.right).Order().ToList();

        var result1 = leftList.Zip(rightList, (l, r) => l > r ? l - r : r - l).Sum();
    ...
    private static string Solution2(string input)
    {
        ...
        var grouping = lines.Select(l => l.right).GroupBy(l => l);
        var result2 = leftList.Select(elem => grouping.SingleOrDefault(g => g.Key == elem)?.ToList().Count * elem ?? 0).Sum();
        return result2.ToString();
    }
    
    static (int left, int right) ParseLine(string line)
    {
        var parts = line
            .Split(" ")
            .Where(part => int.TryParse(part.Trim(), out _))
            .Select(int.Parse)
            .ToArray();
